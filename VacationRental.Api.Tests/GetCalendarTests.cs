...
public class GetCalendarTests
{
    ...
    public async Task GivenCompleteRequest_WhenGetCalendar_ThenAGetReturnsTheCalculatedCalendar()
    {
        ...
        using (var getCalendarResponse = await _client.GetAsync($"/api/v1/calendar?rentalId={postRentalResult.Id}&start=2000-01-01&nights=7"))
        {
            ...
            Assert.Contains(getCalendarResult.Dates[3].PreparationTimes, x => x.Unit == postBooking1Result.Id);

            Assert.Equal(new DateTime(2000, 01, 05), getCalendarResult.Dates[4].Date);
            Assert.Empty(getCalendarResult.Dates[4].Bookings);

            Assert.Contains(getCalendarResult.Dates[4].PreparationTimes, x => x.Unit == postBooking1Result.Id);
            Assert.Contains(getCalendarResult.Dates[4].PreparationTimes, x => x.Unit == postBooking2Result.Id);

            Assert.Equal(new DateTime(2000, 01, 06), getCalendarResult.Dates[5].Date);
            Assert.Contains(getCalendarResult.Dates[5].PreparationTimes, x => x.Unit == postBooking2Result.Id);
